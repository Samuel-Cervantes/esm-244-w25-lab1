...
There are 197618 observations and 12 rows. The data includes tree_id, legal_status, species, address, site order, site info, care taker, plant date, dbh, plot size, latitude and longitude. 

What is the species richness of the trees? Is one species more prevalent than others? Which streets/zip codes have more tree density. 
...
How many counts of each observations by legal_status? Focus on the highest trees
...
plot <-  ggplot() +
  ...
  labs(y = "Tree Count (n)", x = "Legal Status") +
  theme_minimal()+
...
summarize the counts of oak and pines
...
```{r}
scatter_plot <-  ggplot(data = oak_pine_df, aes(x = longitude, y = latitude, color = type))+
 geom_point() +
 theme_minimal() +
  theme(axis.title = element_blank()) +
  labs(color = 'Tree type', 
       caption = 'locations of pine and oak in Sf')
  scatter_plot  
```
